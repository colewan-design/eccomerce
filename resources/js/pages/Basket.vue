<template>
    <v-toolbar flat class="px-0">
      ...
      <v-text-field clearable v-model="table_search" outlined hide-details dense style="maxWidth: 1500px" label="Enter Basket Code" class=""  @change="search"></v-text-field>
      <v-btn color="primary" dark class="ml-3" @change="search"> Search </v-btn>
      <v-divider class="mx-4" inset vertical></v-divider>
      <v-btn small dark color="teal" @click="create_basket()" class="ma-2">
        <v-icon left> mdi-book-variant </v-icon> New Basket
      </v-btn>
      <v-spacer></v-spacer>
    </v-toolbar>
...
>
  ...
  <v-dialog v-model="modal_basket" transition="dialog-top-transition" scrollable persistent max-width="50%">
  <v-card>
    ...
    <v-col cols="12" class="pa-5">
        ...
        <v-select
            label="Select"
            :items="['Big Basket', 'Medium Basket', 'Big Basket']"
  ...
  methods: {
     create_basket() {
      ...
      Save() {
        if (this.id) {
          axios
            .put("/Basket/basket-update/" +this.id, this.basket_crud)
            .then((response) => {
              this.Baskets();
              this.close();
            })
          ...
          axios
            .post("/Basket/basket-store", this.basket_crud)
            .then((response) => {
              this.Baskets();
              this.close();
            })
